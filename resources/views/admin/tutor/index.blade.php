            ...
            <div class="card shadow-sm">
                <div class="card-header bg-gradient-navy d-flex align-items-center">
                    ...
                    <div class="card-tools ml-auto">
                        <a href="{{ route('admin.tutor.create') }}" class="btn btn-success btn-sm shadow">
                            ...
                            <tbody>
                                @foreach ($tutors as $tutor)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="font-weight-bold">{{ $tutor->username }}</span>
                                        </td>
                                        <td>{{ $tutor->name }}</td>
                                        <td class="text-center">
                                            <span
                                                class="badge {{ $tutor->jenis_kelamin == 'L' ? 'badge-primary' : 'badge-pink' }}">
                                                {{ $tutor->jenis_kelamin }}
                                            </span>
                                        </td>
                                        <td>
                                            <a class="text-success font-weight-bold" target="_blank"
                                                href="https://api.whatsapp.com/send?phone=62{{ $tutor->no_wa }}">
                                                <i class="fab fa-whatsapp"></i> {{ $tutor->no_wa }}
                                            </a>
                                        </td>
                                        <td class="text-center">
                                            ...
                                            <button type="button" class="btn btn-warning btn-xs btn-password shadow-sm"
                                                ...
                                                data-nama="{{ $tutor->name }}" data-username="{{ $tutor->username }}"
